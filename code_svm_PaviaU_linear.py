...
def train_svm(data_file, gt_file, b_pca):
    ...
    # svm
    svm_classifier = SVM(LinearKernel())
    ...
    plot_predict_gt_image(gt, y_pred_image,
                          filename=r'C:\pingzhang\csci575_final_project_ping\results\with_pca\svm\SVM_PaviaU_kernel_linear')

    #################################################

    # Save results for SVM.
    f = open(
        r'C:\pingzhang\csci575_final_project_ping\results\with_pca\svm\classify_pavia_U_SVM_results_kernel_linear.dump',
        'wb')
    pickle.dump(
